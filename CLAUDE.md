...
- Call cockpit (`/cockpit`, `demo/cockpit.html`): Cockpit C layout wired to the loaded models, organised around diarized speakers. A transcript source (real scene, library dialogue or paste) plays word by word; speakers are shown as "Person N" (up to nine; the planned live model separates four at once), each with one colour (`--s1` to `--s8` in `app.css`, a ninth is neutral) that links their row, bubble, highlight, cue cards and chart line. Each person's own trailing window is scored at each sentence end (or every N words) through `/api/batch`. Signals belong to a role: everyone else (cue cards), the coached person (coaching: talk share and pace from the clock, recent flags, key points) or the whole call. Signals rise, hold 30 s and fade; validated detectors show measured accuracy, your own sentences are flagged unvalidated. Replay precomputes, Live calls as it goes; the hosted model asks first. The transcript source is just words with a speaker and a time (`buildTimeline`, `runner.feed`), so live speech-to-text can replace it later. `/stream` is unchanged. The mock-ups and their specs are in `demo/mockups/`; `START.md` has the run commands.
