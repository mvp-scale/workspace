...
- Call cockpit (`/cockpit`): Cockpit C layout wired to the loaded models. A transcript source (real scene, library dialogue or paste) plays word by word; each person's own trailing window is scored at each sentence end (or every N words) through `/api/batch`; signals (validated detectors plus your own sentences, flagged unvalidated) rise, hold 30 s and fade; cue cards, coaching (talk share and pace from the clock), a 60-second chart and a log. Replay precomputes, Live calls as it goes; the hosted model asks first. The transcript source is just words with a speaker and a time (`buildTimeline`, `runner.feed`), so live speech-to-text can replace it later. `/stream` is unchanged. Series colours are `--s1` to `--s8` in `app.css`. The mock-ups and their specs are in `demo/mockups/`; `START.md` has the run commands.
