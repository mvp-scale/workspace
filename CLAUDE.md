...
`demo/server.py` (stdlib only) serves a six-page console at `http://127.0.0.1:8100` (Conversation flow at `/flow`, `demo/flow.html`, is described below the list): Leaderboard (`/`), Baseline compare (`/compare`, one question to every model), Scenario lab (`/scenarios`, accuracy and confidence on published labelled sets), Conversation flow (`/flow`), How they work (`/models`) and Report (`/report`). Start with `set -a; . ./.env; set +a; python3 demo/server.py`; without `TYPESAFE_API_KEY` the hosted column reports an error. The key is used server-side only, and the server binds to `127.0.0.1` unless `DEMO_HOST` is set.

- Conversation flow (`/flow`, `demo/flow.html`, formerly the Call cockpit; the Text windows and Call monitor pages were removed, their `/api/speeches` and `/api/window-study` endpoints remain): Cockpit C layout wired to the loaded models. A transcript source (real scene, library dialogue or paste) plays word by word; speakers are diarized voices shown as V1, V2 (up to nine; names appear only for scripted scenes, on the voice cards and thread; the planned live model separates four at once). Detectors (up to 20) each have a colour (`--s1` to `--s8` in `app.css`) and an icon from a built-in set; you add your own with a name, what it should detect and an example (flagged unvalidated, marked `?`). Every detector scores each person's own trailing window at each sentence end (or every N words) through `/api/batch`; the whole-call detector (manipulation) reads the whole call. A detector bar shows level, hold and state; person cards show which detectors each person just hit; cue cards cover everyone except the coached person; coaching for the person you pick is one bold, time-boxed instruction (for example LISTEN, next 30 s) chosen by plain rules on talk share, pace and detector alerts, not a model. The history strip is a heat map (detectors by time) or ticks per person. The Captured card has tabs: Flagged, Detectors (list and add form, measured accuracy), Settings (model, window, cadence, alert), Request (the exact `/v1/systemone` body with a well-formed check, copy and export) and Events (documented events). Replay precomputes, Live calls as it goes; the hosted model asks first. The transcript source is just words with a speaker and a time (`buildTimeline`, `runner.feed`), so live speech-to-text can replace it later. `/stream` is unchanged. The mock-ups and their specs are in `demo/mockups/`; `START.md` has the run commands.
- Conversation flow, live audio (`/flow-lab-final`, `demo/flow-lab-final.html`; `/flow` and `/flow-classic` both still serve the original `flow.html`, and `/flow-lab` is the older lab). One page for Replay (a saved conversation) and Live (real audio): a single Source list (saved conversations, YouTube link, audio file, `data/audio` library file, microphone, tab/call audio), one primary button, and a fixed-size header so switching never re-flows. Live audio goes through the voice server on `:8200` (`voice/ingest.py`: yt-dlp piped into ffmpeg, or uploaded bytes spooled to a temp file; HTTPS YouTube hosts only; 2 h / 300 MB caps; optional `play:true` sends the paced PCM back so the page can play it). `demo/static/voice-source.js` is the append-only stabiliser for the server's snapshots (node tests: `node --test demo/tests/voice-source.test.js`; real-speech fixtures live in git-ignored `data/voice-fixtures/`, never commit third-party audio or transcripts). Hosted Jev scoring in Live is capped per session (default 150 calls, queue trimmed to 4). The thread is neutral bubbles (you right, accent edge, "You"; others left with a colour dot), older lines compact; the Voices row is eight fixed lanes over the last 60 s; cue cards are retired (Flagged tab plus a chip pulse). Coaching is rules plus, in Live, Jev choosing among fixed moves every 15 s (three choice questions, see `coachQs`). Levels tab: every detector against every voice. Replay checkpoint logic is the same as classic (verified by diffing `makeRunner` and by comparing `dryRun` output across 15 scene/window/cadence configs).
- Detector packs and AI drafts: `demo/detectors/*.json|yaml` (see its README; core16, meeting, observation, interview, scam call caller and victim, coaching, board, sales, customer service, negotiation, conflict, operations, debate). `demo/detector_api.py` serves them and drafts detectors from a name using `ANTHROPIC_API_KEY` (server side, off when empty). `demo/audio_library.py` serves `data/audio` read-only with Range support.
...
Models and codes: `demo/models.json` holds each model's stable id, six-character code (SEMIF4, OAJEV4, KEV4B, KEV8H, KEV5H, JEFF4H, LAYA4H, VERD2H, JEV113; letters = family, B = billions of parameters, H = hundreds of millions) and display name. Every page shows models through `Jev.tag(id)` (code chip plus name) or `Jev.codeOf(id)`; never print a raw id such as `so1` or the old `jev (typesafe)` (the hosted backend id is `jev`).

The loaded lineup is managed by `demo/lineup.sh` (`install`, `up`, `down`, `status`): systemd services `kev-4b` (bf16, port 8010, starts first), `semif` (:8012), `so1` (:8013), `laya` (:8014) and `verdict` (:8015, CPU), about 27.5 GiB of the 31.8 GiB card. The last four are `demo/serve_inproc.py`, a standard-library server that wraps the same jevbench adapter that produced each model's benchmark numbers (live and stored answers matched exactly on the items checked). `install` retires the old `kev`, `kev-proxy` and `jeff` services (kev-0.5b, kev-0.8b and jeff no longer fit next to the top models); their stored results remain.

GPU memory: `demo/vram.py` measures each model's real footprint (per-process nvidia-smi under long inputs and the 9-question battery) into `demo/vram.json`; the Leaderboard reads it via `/api/vram`. kev serves in fp32 by default (17 GiB for kev-4b, the numbers behind every benchmark result), which is why the four local servers overran the 32 GiB card. `KEV_DTYPE=bf16` gave the same accuracy on 372 items across four sets (97-100% identical answers) but only saves memory if the load-time fp32 cache is released: launch through `demo/serve_kev.py` (run from `/workspace/kev`), which does that (kev-4b: 8.5 GiB loaded, 9.8 GiB peak). kev-4b still needs about 17.8 GiB while loading, so start it before the other models. The sum of each model's peak is the safe budget; peaks rarely coincide, so an over-budget mix works until a long input arrives.
