...
**Status 2026-09-25 (latest, read this first):** the mock-up phase is done and the final page exists: `/flow-lab-final` (`demo/flow-lab-final.html`), which is `flow.html` plus Live audio, Levels, Counters, detector packs, AI-drafted detectors, voice lanes and Jev-assisted coaching. `/flow` still serves the original; `/flow-classic` is the same file under its future name. **Next**: (1) a paid check of the whole loop on `data/audio/nanobaiter-5min.m4a` with the Scam call caller + victim packs (about 70 detector calls plus about 20 coaching calls, cap 150) to see real scores, wins and coaching; (2) then swap `/flow` to the new page and update the nav and static build (Live cannot run in the static build); (3) still open: the voice server needs a restart to pick up the YouTube 403 retry in `voice/ingest.py`, and `ANTHROPIC_API_KEY` in `.env` is an empty placeholder so name-only detector drafting falls back to a template. Findings so far are in `docs/voice-flow-integration-plan.md` ("Measured so far"). The rest of this section is the original plan, kept for the reasoning.
